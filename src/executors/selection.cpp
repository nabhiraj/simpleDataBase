...
//there are two kinds of select type
//1. coloumn.   COLUMN
//2. int litteral.  INT_LITERAL
void executeSELECTION()
{
    ...
    //optimising the selection in case of linear hashing.
    if(table.indexUsed==HASH
    &&parsedQuery.selectionBinaryOperator==EQUAL
    &&table.indexedColumn==parsedQuery.selectionFirstColumnName
    &&parsedQuery.selectType==INT_LITERAL){
        cout<<"executing from inside the hash"<<endl;
        vector<int> row_ids;
        row_ids=table.lh->search(parsedQuery.selectionIntLiteral);
        int n=row_ids.size();
        for(int i=0;i<n;i++){
            int block_id=row_ids[i]/table.maxRowsPerBlock;
            int local_row_id=row_ids[i]%table.maxRowsPerBlock;
            cursor.nextPage(block_id);
            row=cursor.page.getRow(local_row_id);
            resultantTable->writeRow<int>(row);//should work.
        }
    }else{
        while (!row.empty()){
